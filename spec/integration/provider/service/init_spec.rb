...
describe provider, :'fails_on_ruby_1.9.2' => true do
  describe "when running on FreeBSD", :if => (Puppet::Node::Facts["operatingsystem"] == "FreeBSD") do
  ...
  describe "when running on HP-UX", :if => (Puppet::Node::Facts["operatingsystem"] == "HP-UX") do
    it "should set its default path to include /sbin/init.d" do
      provider.defpath.should == "/sbin/init.d"
    end
  end

  describe "when not running on FreeBSD or HP-UX", :if => (! %w{HP-UX FreeBSD}.include?(Puppet::Node::Facts["operatingsystem"])) do
