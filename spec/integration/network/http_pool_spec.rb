...
class SSLTestCase
  ...
  def start_server(ctx_proc: nil, server_proc: method(:readwrite_loop), &block)
    errors = []
    ...
    IO.pipe {|stop_pipe_r, stop_pipe_w|
      ...
      ctx.tmp_dh_callback = proc { OpenSSL::PKey::DH.new(DHPARAMS) }
...
describe Puppet::Network::HttpPool do
  ...
  let(:server) { SSLTestCase.new }
  let(:ssl) { Puppet::SSL::SSLProvider.new }
  let(:ca_cert) { OpenSSL::X509::Certificate.new(CA_CERT) }
  let(:server_key) { OpenSSL::PKey::RSA.new(SERVER_KEY) }
  let(:server_cert) { OpenSSL::X509::Certificate.new(SERVER_CERT) }
  let(:config) { WEBrick::Config::HTTP.dup }
  let(:server_proc) {
    -> (ctx, ssl) {
  ...
  let(:requests_client_certs) {
    -> (server_ctx) {
      server_ctx.verify_mode = OpenSSL::SSL::VERIFY_PEER
    }
  }
  let(:requires_client_certs) {
    -> (server_ctx) {
      server_ctx.verify_mode = OpenSSL::SSL::VERIFY_PEER | OpenSSL::SSL::VERIFY_FAIL_IF_NO_PEER_CERT
    }
  }
  let(:host) { '127.0.0.1' }
  let(:wrong_host) { 'localhost' }

  def connection(host, port, ssl_context)
    Puppet::Network::HttpPool.connection(URI("https://#{host}:#{port}"), ssl_context: ssl_context)
  end

  context 'when negotiating the connection' do
    let(:ctx) { ssl.create_root_context(cacerts: [ca_cert], revocation: false) }

    it 'rejects SSLv3' do
      ctx_proc = -> (server_ctx) {
        server_ctx.ssl_version = :SSLv3_server
      }

      server.start_server(ctx_proc: ctx_proc, server_proc: server_proc) do |port|
        http = connection(host, port, ctx)

        expect {
          http.get('/')
        }.to raise_error(OpenSSL::SSL::SSLError, /unsupported protocol/)
      end
    end
  end

  context 'when the client has a cert' do
    let(:ctx) { ssl.create_context(cacerts: [ca_cert], crls: [], revocation: false, private_key: server_key, client_cert: server_cert) }

    it 'performs mutual authentication' do
      client_cert = nil

      server_cb = -> (ctx, ssl) {
        client_cert = ssl.peer_cert
        server_proc.call(ctx, ssl)
      }

      server.start_server(ctx_proc: requires_client_certs, server_proc: server_cb) do |port|
        http = connection(host, port, ctx)
        http.get('/')
        expect(client_cert).to be_an_instance_of(OpenSSL::X509::Certificate)
      end
    end
  end

  context 'when the client does not have a cert' do
    let(:ctx) { ssl.create_root_context(cacerts: [ca_cert], crls: [], revocation: false) }

    it 'connects if the server requests, but does not require one' do
      server.start_server(ctx_proc: requests_client_certs, server_proc: server_proc) do |port|
        http = connection(host, port, ctx)
        http.get('/')
      end
    end

    it 'raises if client certs are required' do
      server.start_server(ctx_proc: requires_client_certs, server_proc: server_proc) do |port|
        http = connection(host, port, ctx)
        expect {
          http.get('/')
        }.to raise_error(OpenSSL::SSL::SSLError, /sslv3 alert handshake failure/)
      end
    end

    it "raises if the server's hostname doesn't match its cert" do
      server.start_server(server_proc: server_proc) do |port|
        http = connection(wrong_host, port, ctx)
        expect {
          http.get('/')
        }.to raise_error(
               Puppet::SSL::CertMismatchError,
               "Server hostname 'localhost' did not match server certificate; expected 127.0.0.1"
             )
      end
    end
  end

  context 'when the client does not verify the server' do
    it "connects to a server whose cert doesn't match" do
      ctx = ssl.create_insecure_context
