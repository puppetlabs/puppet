...
describe "Puppet defaults" do
  ...
  describe "properly enforces permissions on" do
    describe "vardir" do
      before :each do
        # Puppet will create this dir with requested permissions
        @vardir = Puppet::FileSystem.pathname(Puppet.settings[:vardir])
        @path = @vardir.to_s

        # instruct Puppet to load settings, which resets perms
        Puppet.settings.use(:main)
      end

      describe "on POSIX", :if => Puppet.features.root? &&
        !Puppet.features.microsoft_windows? do

        it "with a group and owner of service" do
          provider = Puppet::Type.type(:file).provider(:posix).new

          stat = Puppet::FileSystem.stat(@path)
          provider.uid2name(stat.uid).should == Puppet.settings[:user]
          provider.gid2name(stat.gid).should == Puppet.settings[:group]
        end
      end

      describe "on Windows", :if => Puppet.features.root? &&
        Puppet.features.microsoft_windows? do

        it "with a group and owner of service" do
          sd = Puppet::Util::Windows::Security.get_security_descriptor(@path)

          sd.owner.should == Win32::Security::SID::BuiltinAdministrators
          sd.group.should == Win32::Security::SID::Service
        end
      end
    end

    # really, any file that relies on 'root' would do here
    # we simply want to verify that a file with 'root' owner and mode works
    describe "classfile" do

      before :each do
        @classfile = Puppet::FileSystem.pathname(Puppet.settings[:classfile])
        @path = @classfile.to_s

        FileUtils.mkdir_p(@classfile.parent)
      ...
      describe "on POSIX", :if => Puppet.features.root? &&
        ...
        it "with an owner of root" do
          provider = Puppet::Type.type(:file).provider(:posix).new

          # change owner to nobody
          nobody_id = provider.name2uid('nobody')
          File.chown(nobody_id, nil, @path)

          # instruct Puppet to load settings, which resets perms
          Puppet.settings.use(:agent)

          provider.uid2name(classfile.stat.uid).should == 'root'
          provider.mode.should == '640'
        end
      end
      ...
      describe "on Windows", :if => Puppet.features.root? &&
        ...
        it "with an owner of Administrators" do
          # change the owner to everyone
          sd = Puppet::Util::Windows::Security.get_security_descriptor(@path)
          sd.owner = Win32::Security::SID::Everyone
          ...
          mode = Puppet::Util::Windows::Security.get_mode(@path)
          mode.to_s(8).should == '640'
        end
      end
    end
  end
