  ...
  shared_examples_for "a securable object" do
    ...
    describe "on a volume that supports ACLs" do
      describe "for a normal user" do
        ...
        describe "#group= and #owner=" do
          it "should not remove the full access permission entry for SYSTEM on a file or directory" do
            [path, dir].each do |target|
              # new file has SYSTEM
              system_aces = winsec.get_aces_for_path_by_sid(target, sids[:system])
              system_aces.should_not be_empty
              system_aces.each { |ace| ace[:mask].should == WindowsSecurityTester::FILE_ALL_ACCESS }

              winsec.set_group(sids[:power_users], target)
              winsec.set_owner(sids[:current_user], target)

              # and should still have the same SYSTEM ACE(s)
              winsec.get_aces_for_path_by_sid(target, sids[:system]).should == system_aces
            end
          end
        end
