...
describe Puppet::SSL::CertificateFactory do
  ...
  subject { Puppet::SSL::CertificateFactory }
  ...
  describe "when generating the certificate" do
    ...
    it "should have 24 hours grace on the start of the cert" do
      cert = subject.build(:server, csr, issuer, serial)
      (cert.not_before - (Time.now - 24*60*60)).abs.should >= 0
      (cert.not_before - (Time.now - 24*60*60)).abs.should <= 1
    end

    it "should set the default TTL of the certificate" do
      ttl  = Puppet::SSL::CertificateFactory.ttl
      cert = subject.build(:server, csr, issuer, serial)
      (cert.not_after - (Time.now + ttl)).abs.should >= 0
      (cert.not_after - (Time.now + ttl)).abs.should <= 1
    end

    it "should respect a custom TTL for the CA" do
      Puppet[:ca_ttl] = 12
      cert = subject.build(:server, csr, issuer, serial)
      (cert.not_after - (Time.now + 12)).abs.should >= 0
      (cert.not_after - (Time.now + 12)).abs.should <= 1
    end
    ...
    it "should not add subjectAltNames when it isn't configured" do
      # Force creation of the CSR, and check it has no extReq
      Puppet[:certdnsnames] = ''
    ...
    it "should add subjectAltName when the CSR requests them" do
      ...
      csr.subject_alt_names.should =~ %w{example.local one two}.map{|x| "DNS:#{x}"}

      Puppet[:certdnsnames] = ''
      cert = subject.build(:server, csr, issuer, serial)
      san = cert.extensions.find {|x| x.oid == 'subjectAltName' }
      san.should_not be_nil
      %w{one two example.local}.each do |name|
