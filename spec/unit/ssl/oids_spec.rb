...
describe Puppet::SSL::Oids do
  ...
  describe "checking if an OID is a subtree of another OID" do
    ...
    it "can determine if an OID is contained in another OID" do
      described_class.subtree_of?('1.3.6.1', '1.3.6.1.4.1').should be_truthy
      described_class.subtree_of?('1.3.6.1.4.1', '1.3.6.1').should be_falsey
    end

    it "returns true if an OID is compared against itself and exclusive is false" do
      described_class.subtree_of?('1.3.6.1', '1.3.6.1', false).should be_truthy
    end

    it "returns false if an OID is compared against itself and exclusive is true" do
      described_class.subtree_of?('1.3.6.1', '1.3.6.1', true).should be_falsey
    end

    it "can compare OIDs defined as short names" do
      described_class.subtree_of?('IANA', '1.3.6.1.4.1').should be_truthy
      described_class.subtree_of?('1.3.6.1', 'enterprises').should be_truthy
    end

    it "returns false when an invalid OID shortname is passed" do
      described_class.subtree_of?('IANA', 'bananas').should be_falsey
    end
  end
end
