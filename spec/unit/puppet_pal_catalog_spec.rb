...
describe 'Puppet Pal' do
  ...
  context 'with code in modules and env' do
    ...
    context 'configured as existing given envpath such that' do
      ...
      context 'when catalog compiler is used callbacks are supported' do
        it 'such that compilation and environment end callbacks are performed' do
          ...
          Puppet::Pal.in_environment('pal_env', envpath: environments_dir, facts: node_facts) do |ctx|
            ctx.with_catalog_compiler(configured_by_env: true) {|c|  c.call_function('a::a_caching_func', guard)}
          end
