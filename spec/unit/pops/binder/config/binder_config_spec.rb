...
describe 'BinderConfig' do
  ...
  it 'should load default config if no config file exists' do
    ...
    expect(config.categorization.size).to be == 5
    expect(config.categorization[0][0]).to be == 'node'
    expect(config.categorization[1][0]).to be == 'operatingsystem'
    expect(config.categorization[2][0]).to be == 'osfamily'
    expect(config.categorization[3][0]).to be == 'environment'
    expect(config.categorization[4][0]).to be == 'common'
  end
  ...
  it 'should load binder_config.yaml with different types of category entries' do
    Puppet::Pops::Binder::Config::BinderConfig.any_instance.stubs(:confdir).returns(my_fixture("/ok_mixed_categories/"))
    config = Puppet::Pops::Binder::Config::BinderConfig.new(diag)
    expect(acceptor.errors?()).to be == false
    expect(config.layering_config[0]['name']).to    be == 'site'
    expect(config.layering_config[0]['include']).to be == 'confdir-hiera:/'
