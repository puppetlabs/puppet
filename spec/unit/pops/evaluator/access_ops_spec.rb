...
describe 'Puppet::Pops::Evaluator::EvaluatorImpl/AccessOperator' do
  ...
  context 'The evaluator when operating on a String' do
    it 'can get a single character using a single key index to []' do
      expect(evaluate(literal('abc').test_access(1))).to eql('b')
    end

    it 'can get the last character using the key -1 in []' do
      expect(evaluate(literal('abc').test_access(-1))).to eql('c')
    end

    it 'can get a substring by giving two keys' do
      expect(evaluate(literal('abcd').test_access(1,2))).to eql('bc')
      # flattens keys
      expect(evaluate(literal('abcd').test_access([1,2]))).to eql('bc')
    end

    it 'produces empty string for a substring out of range' do
      expect(evaluate(literal('abc').test_access(100))).to eql('')
    end

    it 'raises an error if arity is wrong for []' do
      expect{evaluate(literal('abc').test_access)}.to raise_error(/String supports \[\] with one or two arguments\. Got 0/)
      expect{evaluate(literal('abc').test_access(1,2,3))}.to raise_error(/String supports \[\] with one or two arguments\. Got 3/)
    end
  end

  context 'The evaluator when operating on a Binary' do
    it 'can get a single character using a single key index to []' do
      expect(evaluate(binary('abc').test_access(1)).binary_buffer).to eql('b')
    end

    it 'can get the last character using the key -1 in []' do
      expect(evaluate(binary('abc').test_access(-1)).binary_buffer).to eql('c')
    end

    it 'can get a substring by giving two keys' do
      expect(evaluate(binary('abcd').test_access(1,2)).binary_buffer).to eql('bc')
      # flattens keys
      expect(evaluate(binary('abcd').test_access([1,2])).binary_buffer).to eql('bc')
    end

    it 'produces empty string for a substring out of range' do
      expect(evaluate(binary('abc').test_access(100)).binary_buffer).to eql('')
    end

    it 'raises an error if arity is wrong for []' do
      expect{evaluate(binary('abc').test_access)}.to raise_error(/String supports \[\] with one or two arguments\. Got 0/)
      expect{evaluate(binary('abc').test_access(1,2,3))}.to raise_error(/String supports \[\] with one or two arguments\. Got 3/)
    end
  end

  context 'The evaluator when operating on an Array' do
    it 'is tested with the correct assumptions' do
      expect(literal([1,2,3]).test_access(1).model_class <= Puppet::Pops::Model::AccessExpression).to eql(true)
    end

    it 'can get an element using a single key index to []' do
      expect(evaluate(literal([1,2,3]).test_access(1))).to eql(2)
    end

    it 'can get the last element using the key -1 in []' do
      expect(evaluate(literal([1,2,3]).test_access(-1))).to eql(3)
    end

    it 'can get a slice of elements using two keys' do
      expect(evaluate(literal([1,2,3,4]).test_access(1,2))).to eql([2,3])
      # flattens keys
      expect(evaluate(literal([1,2,3,4]).test_access([1,2]))).to eql([2,3])
    end

    it 'produces nil for a missing entry' do
      expect(evaluate(literal([1,2,3]).test_access(100))).to eql(nil)
    end

    it 'raises an error if arity is wrong for []' do
      expect{evaluate(literal([1,2,3,4]).test_access)}.to raise_error(/Array supports \[\] with one or two arguments\. Got 0/)
      expect{evaluate(literal([1,2,3,4]).test_access(1,2,3))}.to raise_error(/Array supports \[\] with one or two arguments\. Got 3/)
    end
  end

  context 'The evaluator when operating on a Hash' do
    it 'can get a single element giving a single key to []' do
      expect(evaluate(literal({'a'=>1,'b'=>2,'c'=>3}).test_access('b'))).to eql(2)
    end

    it 'can lookup an array' do
      expect(evaluate(literal({[1]=>10,[2]=>20}).test_access([2]))).to eql(20)
    end

    it 'produces nil for a missing key' do
      expect(evaluate(literal({'a'=>1,'b'=>2,'c'=>3}).test_access('x'))).to eql(nil)
    end

    it 'can get multiple elements by giving multiple keys to []' do
      expect(evaluate(literal({'a'=>1,'b'=>2,'c'=>3, 'd'=>4}).test_access('b', 'd'))).to eql([2, 4])
    end

    it 'compacts the result when using multiple keys' do
      expect(evaluate(literal({'a'=>1,'b'=>2,'c'=>3, 'd'=>4}).test_access('b', 'x'))).to eql([2])
    end

    it 'produces an empty array if none of multiple given keys were missing' do
      expect(evaluate(literal({'a'=>1,'b'=>2,'c'=>3, 'd'=>4}).test_access('x', 'y'))).to eql([])
    end

    it 'raises an error if arity is wrong for []' do
      expect{evaluate(literal({'a'=>1,'b'=>2,'c'=>3}).test_access)}.to raise_error(/Hash supports \[\] with one or more arguments\. Got 0/)
    end
  end
  ...
  context "When applied to a type it" do
    ...
    it 'produces an Integer[from, to]' do
      expr = fqr('Integer').test_access(1, 3)
    ...
    it 'produces an error for Integer[] if there are more than 2 keys' do
      expr = fqr('Integer').test_access(1,2,3)
    ...
    it 'produces a Float[from, to]' do
      expr = fqr('Float').test_access(1, 3)
    ...
    it 'produces a Hash[Scalar,String] from the expression Hash[Scalar, String]' do
      expr = fqr('Hash').test_access(fqr('Scalar'), fqr('String'))
    ...
    it 'gives an error if only one type is specified ' do
      expr = fqr('Hash').test_access(fqr('String'))
    ...
    it 'produces a Hash[Scalar,String] from the expression Hash[Integer, Array][Integer, String]' do
      expr = fqr('Hash').test_access(fqr('Integer'), fqr('Array')).test_access(fqr('Integer'), fqr('String'))
    ...
    it "gives an error if parameter is not a type" do
      expr = fqr('Hash').test_access('String')
    ...
    it 'produdes a Timespan type with a lower bound' do
      expr = fqr('Timespan').test_access({fqn('hours') => literal(3)})
      expect(evaluate(expr)).to be_the_type(types.timespan({'hours' => 3}))
    end

    it 'produdes a Timespan type with an upper bound' do
      expr = fqr('Timespan').test_access(literal(:default), {fqn('hours') => literal(9)})
    ...
    it 'produdes a Timespan type with both lower and upper bounds' do
      expr = fqr('Timespan').test_access({fqn('hours') => literal(3)}, {fqn('hours') => literal(9)})
    ...
    it 'produdes a Timestamp type with a lower bound' do
      expr = fqr('Timestamp').test_access(literal('2014-12-12T13:14:15 CET'))
    ...
    it 'produdes a Timestamp type with an upper bound' do
      expr = fqr('Timestamp').test_access(literal(:default), literal('2016-08-23T17:50:00 CET'))
    ...
    it 'produdes a Timestamp type with both lower and upper bounds' do
      expr = fqr('Timestamp').test_access(literal('2014-12-12T13:14:15 CET'), literal('2016-08-23T17:50:00 CET'))
    ...
    it 'produces a Tuple[String] from the expression Tuple[String]' do
      expr = fqr('Tuple').test_access(fqr('String'))
    ...
    it "Tuple parameterization gives an error if parameter is not a type" do
      expr = fqr('Tuple').test_access('String')
    ...
    it 'produces a varargs Tuple when the last two arguments specify size constraint' do
      expr = fqr('Tuple').test_access(fqr('String'), 1)
      expected_t = types.tuple([String], types.range(1, :default))
      expect(evaluate(expr)).to be_the_type(expected_t)

      expr = fqr('Tuple').test_access(fqr('String'), 1, 2)
    ...
    it 'creates a PPatternType instance when applied to a Pattern' do
      regexp_expr = fqr('Pattern').test_access('foo')
    ...
    it 'creates a Regexp instance when applied to a Pattern' do
      regexp_expr = fqr('Regexp').test_access('foo')
    ...
    it 'produces a specific class from Class[classname]' do
      expr = fqr('Class').test_access(fqn('apache'))
      expect(evaluate(expr)).to be_the_type(types.host_class('apache'))
      expr = fqr('Class').test_access(literal('apache'))
      expect(evaluate(expr)).to be_the_type(types.host_class('apache'))
    end
    ...
    it 'raises error if access is to no keys' do
      expr = fqr('Class').test_access(fqn('apache')).test_access
    ...
    it 'produces a collection of classes when multiple class names are given' do
      expr = fqr('Class').test_access(fqn('apache'), literal('nginx'))
    ...
    it 'produces an empty array if the keys reduce to empty array' do
      expr = fqr('Class').test_access(literal([[],[]]))
      expect(evaluate(expr)).to be_eql([])
    end

    # Resource
    it 'produces a specific resource type from Resource[type]' do
      expr = fqr('Resource').test_access(fqr('File'))
      expect(evaluate(expr)).to be_the_type(types.resource('File'))
      expr = fqr('Resource').test_access(literal('File'))
      expect(evaluate(expr)).to be_the_type(types.resource('File'))
    end
    ...
    it 'produces a specific resource reference type from File[title]' do
      expr = fqr('File').test_access(literal('/tmp/x'))
    ...
    it 'produces a collection of specific resource references when multiple titles are used' do
      ...
      expr = fqr('File').test_access(literal('x'),literal('y'))
      result = evaluate(expr)
      ...
      expr = fqr('Resource').test_access(fqr('File'), literal('x'),literal('y'))
      result = evaluate(expr)
      expect(result[0]).to be_the_type(types.resource('File', 'x'))
      expect(result[1]).to be_the_type(types.resource('File', 'y'))
    end
    ...
    it 'produces an empty array if the keys reduce to empty array for a specific Resource tyoe' do
      expr = fqr('File').test_access(literal([[],[]]))
      expect(evaluate(expr)).to be_eql([])
    end

    it 'gives an error if resource is not found' do
      expr = fqr('File').test_access(fqn('x')).test_access(fqn('y'))
    ...
    it 'produces a NotUndef instance with contained type' do
      type_expr = fqr('NotUndef').test_access(fqr('Integer'))
    ...
    it 'produces a NotUndef instance with String type when given a literal String' do
      type_expr = fqr('NotUndef').test_access(literal('hey'))
      tf = Puppet::Pops::Types::TypeFactory
    ...
    it 'Produces Optional instance with String type when using a String argument' do
      type_expr = fqr('Optional').test_access(literal('hey'))
      tf = Puppet::Pops::Types::TypeFactory
    ...
    it 'creates a Type instance when applied to a Type' do
      type_expr = fqr('Type').test_access(fqr('Integer'))
      tf = Puppet::Pops::Types::TypeFactory
    ...
    it 'creates a Ruby Type instance when applied to a Ruby Type' do
      type_expr = fqr('Runtime').test_access('ruby','String')
      tf = Puppet::Pops::Types::TypeFactory
