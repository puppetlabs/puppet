...
describe Puppet::Pops::Impl::Model::AstTransformer do
  include FactoryRspecHelper

  let(:filename) { "the-file.pp" }
  let(:transformer) { Puppet::Pops::Impl::Model::AstTransformer.new(filename) }
  ...
  context "literal numbers" do
    ...
    it "converts an unknown radix to an error string" do
      ast = transform(Puppet::Pops::Impl::Model::Factory.new(Puppet::Pops::API::Model::LiteralNumber, 3, 2))
  ...
  def location(line, column, offset)
    position = Puppet::Pops::API::Adapters::SourcePosAdapter.new
    position.line = line
    position.pos = column
