...
describe Puppet::SSL::Key::File do
  ...
  describe "when managing private keys" do
    ...
    it "should destroy the public key when destroying the private key" do
      Puppet::FileSystem.expects(:unlink).with(@private_key_path)
      Puppet::FileSystem.expects(:exist?).with(@private_key_path).returns true
      Puppet::FileSystem.expects(:exist?).with(@public_key_path).returns true
      Puppet::FileSystem.expects(:unlink).with(@public_key_path)
    ...
    it "should not fail if the public key does not exist when deleting the private key" do
      ...
      Puppet::FileSystem.stubs(:exist?).with(@private_key_path).returns true
      Puppet::FileSystem.expects(:exist?).with(@public_key_path).returns false
      Puppet::FileSystem.expects(:unlink).with(@public_key_path).never
