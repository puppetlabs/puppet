  ...
  context "should be callable as" do
    it 'reduce on an array' do
      catalog = compile_to_catalog(<<-MANIFEST)
        ...
        file { "/file_$b": making_sure => present }
      MANIFEST
    ...
    it 'reduce on a hash' do
      catalog = compile_to_catalog(<<-MANIFEST)
        ...
        file { "/file_${$b[0]}_${$b[1]}": making_sure => present }
      MANIFEST

      catalog.resource(:file, "/file_sum_6")['making_sure'].should == 'present'
    end
    it 'reduce on a hash with start value' do
      ...
      catalog.resource(:file, "/file_sum_10")['making_sure'].should == 'present'
    end
  end
end
