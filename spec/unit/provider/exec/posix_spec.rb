  ...
  describe "#run" do
    ...
    describe "when the command is a relative path" do
      it "should execute the command if it finds it in the path and is executable" do
        ...
        Puppet::Util::Execution.expects(:execute).with { |cmdline, arguments| (cmdline == filename) && (arguments.is_a? Hash) }.returns(Puppet::ProcessOutput.new('', 0))
    ...
    it "should execute the command if the command given includes arguments or subcommands" do
      ...
      Puppet::Util::Execution.expects(:execute).with { |cmdline, arguments| (cmdline == "#{command} bar --sillyarg=true --blah") && (arguments.is_a? Hash) }.returns(Puppet::ProcessOutput.new('', 0))
    ...
    it "should warn if you're overriding something in environment" do
      ...
      Puppet::Util::Execution.expects(:execute).with { |cmdline, arguments| (cmdline == command) && (arguments.is_a? Hash) }.returns(Puppet::ProcessOutput.new('', 0))
