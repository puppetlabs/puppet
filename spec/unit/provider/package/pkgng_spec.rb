...
describe provider_class do
  ...
  before do
    ...
    info = File.read('spec/fixtures/unit/provider/package/pkgng/pkg.info')
    provider_class.stubs(:get_query) { info }

    version_list = File.read('spec/fixtures/unit/provider/package/pkgng/pkg.version')
    provider_class.stubs(:get_version_list) { version_list }
  end

  context "#instances" do
    it "should return the empty set if no packages are listed" do
      provider_class.stubs(:get_query) { '' }
      provider_class.stubs(:get_version_list) { '' }
  ...
  context "#query" do
    ...
    it "should return nil if not present" do
      fixture = File.read('spec/fixtures/unit/provider/package/pkgng/pkg.query_absent')
      provider_class.stubs(:get_resource_info).with('bash') { fixture }
