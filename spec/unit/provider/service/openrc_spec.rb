...
describe 'Puppet::Type::Service::Provider::Openrc', unless: Puppet::Util::Platform.jruby? do
  ...
  before :each do
    ...
    ['/sbin/rc-service', '/bin/rc-status', '/sbin/rc-update'].each do |command|
      ...
      provider_class.stubs(:which).with(command).returns(command)
  ...
  describe ".instances" do
    ...
    it "should get a list of services from rc-service --list" do
      provider_class.expects(:rcservice).with('-C','--list').returns File.read(my_fixture('rcservice_list'))
  ...
  describe "#enabled?" do
    ...
    before :each do
      provider_class.any_instance.stubs(:rcstatus).with('-C','-a').returns File.read(my_fixture('rcstatus'))
    end
