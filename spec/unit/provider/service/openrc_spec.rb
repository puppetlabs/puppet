...
describe Puppet::Type.type(:service).provider(:openrc) do
  ...
  before :each do
    ...
    ['/sbin/rc-service', '/bin/rc-status', '/sbin/rc-update'].each do |command|
      ...
      allow(described_class).to receive(:which).with(command).and_return(command)
  ...
  describe ".instances" do
    ...
    it "should get a list of services from rc-service --list" do
      expect(described_class).to receive(:rcservice).with('-C','--list').and_return(File.read(my_fixture('rcservice_list')))
  ...
  describe "#enabled?" do
    before :each do
      allow_any_instance_of(described_class).to receive(:rcstatus).with('-C','-a').and_return(File.read(my_fixture('rcstatus')))
    end
