...
to execute them through the rake task.
...
## Source Repository Available as a Local Gem ##

Alternatively, a checkout of the source repository can be configured in a way
that provides puppet as a gem rather than a simple Ruby library.  The parent
directory should be set along the `GEM_PATH`, preferably before other tools
such as RVM that manage gemsets using `GEM_PATH`.

This requires a gemspec, which is an artifact generated by the packaging system
used for puppet and other puppetlabs projects.

For example, with Puppet checked out into `/workspace/src/puppet` using `git
checkout https://github.com/puppetlabs/puppet` in `/workspace/src`, we first
need to:

1) get a current gemspec:

    $ rake package:bootstrap # clones the packing repo into ./ext
    $ rake package:gem       # creates a gem of the local puppet repo in ./pkg
    $ gem specification --ruby pkg/<name-of-puppet-gem-created-in-previous-step>.gem > puppet.gemspec

This last line extracts the gem specification from the gem and places it locally in the file 'puppet.gemspec'.

Next, prepare the workspace to be picked up in the current user's gems list.
The trick is to symlink `gems` to `src` and ensure that puppet is found in the GEM_PATH.
