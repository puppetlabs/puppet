...
Puppet::Type.type(:service).provide :smf, :parent => :base do
  # TODO: Update the documentation with things that you find.
  # E.g. About specifying fmri explicitly in the manifest.
  desc <<-EOT
  ...
  def initialize(*args)
    ...
    # This hash contains the properties we need to sync. in our flush method. We use a separate hash
    # instead of @property_hash to simplify its implementation, since the latter stores ALL of the
    # specified properties, not just the ones that need to be syncd.
    @properties_to_sync = {}
  end
  ...
  # Returns the service's FMRI. We fail if multiple FMRIs correspond to @resource[:name].
  #
  # If the service does not exist or we fail to get any FMRIs from svcs, this method
  # will raise a Puppet::ExecutionFailure
  def service_fmri
    return @fmri if @fmri

    # `svcs -l` is better to use because we can detect service instances that have not
    # yet been activated or enabled (that svcadm has not yet touched). `svcs -H -o fmri`
    ...
    unless fmris.length == 1
      raise Puppet::Error.new(
        "Failed to get #{@resource[:name]}'s FMRI: The pattern '#{@resource[:name]}' matches multiple FMRIs!"
      )
    end
