...
class PNTransformer
  ...
  def transform_AccessExpression(e)
    PN::List.new([transform(e.left_expr)] + pn_array(o.keys)).with_name('access')
  end
  ...
  def transform_AttributesOperation(e)
    PN::Call.new('splat_hash', transform(e.expr))
  end
  ...
  def transform_CaseExpression(e)
    PN::Call.new('case', transform(e.test), pn_array(e.options))
  end
  ...
  def transform_CollectExpression(e)
    entries = [transform(o.type_expr).with_name('type'), transform(e.query).with_name('query')]
  ...
  def transform_HeredocExpression(e)
    entries = []
    entries << PN::Literal.new(e.syntax) unless e.syntax.nil?
  ...
  def transform_LambdaExpression(e)
    ...
    PN::Map.new(entries).with_name('lambda')
  end
  ...
  def transform_LiteralInteger(e)
    ...
    e.radix == 10 ? vl : PN::Map.new([PN::Literal.new(e.radix).with_name('radix'), vl]).as_call('int')
  end
  ...
  def transform_LiteralRegularExpression(e)
    PN::Literal(e.value).as_call('regexp')
  end
  ...
  def transform_NodeDefinition(e)
    ...
    entries << block_as_entry('body', e.block) unless e.block.nil?
  ...
  def transform_QualifiedName(e)
    PN::Call.new('qn', transform(e.value))
  end

  def transform_QualifiedReference(e)
    PN::Call.new('qr', transform(e.cased_value))
  end
  ...
  def transform_ResourceExpression(e)
    entries = [
      ...
      PN::List.new(e.bodies.map { |body| transform(body).as_parameters }).with_name('bodies')
    ]
    entries << PN::Literal.new(e.form).with_name('form') unless e.form == 'regular'
  ...
  def transform_TypeMapping(e)
    PN::Call.new('type-mapping', transform(e.type_expr), transform(o.mapping_expr))
  end
  ...
  def transform_VariableExpression(e)
    PN::Call.new('var', transform(e.expr))
  end
  ...
  def block_as_entry(name, expr)
    if expr.is_a?(BlockExpression)
      transform(block.statements).with_name(name)
    else
  ...
  def if_to_pn(e, name)
    ...
    entries << transform(e.then_expr).with_name('then') unless is_nop?(e.then_expr)
    entries << transform(e.else_expr).with_name('else') unless is_nop?(e.else_expr)
    PN::Map.new(entries).with_name(name)
  end
end
