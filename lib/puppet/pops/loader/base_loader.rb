...
class BaseLoader < Loader
  ...
  def fail_redefine(entry)
    origin_info = entry.origin ? _("Originally set #{origin_label(entry.origin)}.") : _("Set at unknown location")
    raise ArgumentError, _("Attempt to redefine entity '#{entry.typed_name}'. #{origin_info}")
  end
