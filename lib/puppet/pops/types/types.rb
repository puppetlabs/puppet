...
class TypedModelObject < Object
  ...
  def self.create_ptype(loader, ir, parent_name, attributes_hash = EMPTY_HASH)
    @type = Pcore::create_object_type(loader, ir, self, "Pcore::#{simple_name}Type".freeze, "Pcore::#{parent_name}", attributes_hash)
  end

  def self.register_ptypes(loader, ir)
    types = []
    Types.constants.map do |c|
      cls = Types.const_get(c)
      types << cls.register_ptype(loader, ir) if cls.is_a?(Class) && cls < self
    end
...
class PAnyType < TypedModelObject
  ...
  def writer
    @writer ||= Serialization::Poro::StrictWriter.new(self.class)
  end
end
