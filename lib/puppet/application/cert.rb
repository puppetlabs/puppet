...
class Puppet::Application::Cert < Puppet::Application
  ...
  option("--[no-]allow-dns-alt-names") do |value|
    options[:allow_subject_alt_names] = value
    Puppet.deprecation_warning(_("--allow-dns-alt-names is deprecated and has been replaced with --allow-subject-alt-names."))
  end

  option("--[no-]allow-subject-alt-names") do |value|
    options[:allow_subject_alt_names] = value
  end
...
* --allow-subject-alt-names:
  Sign a certificate request even if it contains one or more alternate
  ...
  alternate names, since Puppet agent relies on those names for identifying
  its rightful server.

  You can make Puppet agent request a certificate with alternate names by
  setting 'subject_alt_names' in puppet.conf or specifying '--subject_alt_names' on the
...
Copyright (c) 2011 Puppet Inc., LLC Licensed under the Apache 2.0 License
  ...
  def setup
    ...
    if subcommand == :generate
      ...
      if Puppet.settings.set_by_cli?(:subject_alt_names)
        options[:subject_alt_names] = Puppet[:subject_alt_names]
      end
    end
