...
puppet resource [-h|--help] [-d|--debug] [-v|--verbose] [-e|--edit]
  [-p|--param <parameter>] [--target DEVICE] [-t|--types] [-y|--to_yaml] <type>
...
* --target:
  Target a specific device in device.conf. Doing so will list resources for only
  that device.
    ...
    Puppet.override(:current_environment => env, :loaders => Puppet::Pops::Loaders.new(env)) do
      ...
      if options[:target]
        ...
        if target_device.nil?
          raise _("Invalid target '%{target}'") % { target: options[:target]}
        end
        # Facter(:url) is used by the device module loaded via
        # Puppet::Resource.indirection.search in find_or_save_resources()
        url = target_device.url.to_s
        Facter.add(:url) do
          setcode { url }
        end
      end
