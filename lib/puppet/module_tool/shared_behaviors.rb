module Puppet::Module::Tool::Shared
  class NoVersionsSatisfyError < StandardError
    def v(version)
    ...
    def multiline
      ...
      msg = []
      msg << "Could not #{@action} module '#{@requested_name}' (#{@vstring})"
      msg << "  No version of '#{@dependency_name}' will satisfy dependencies"
      msg << "    You specified '#{@requested_name}' (#{@requested_version})" if same_mod
      msg += @conditions.select { |c| c[:module] != :you }.sort_by { |c| c[:module] }.map do |c|
        "    '#{c[:module]}' (#{v(c[:version])}) requires '#{@dependency_name}' (#{v(c[:dependency])})"
      end
      msg << "    Use `puppet module #{@action} --force` to #{@action} this module anyway" if same_mod
      msg << "    Use `puppet module #{@action} --ignore-dependencies` to #{@action} only this module" unless same_mod

      msg.join("\n")
    end
  end

  class InvalidDependencyCycleError < StandardError
    def v(version)
      (version || '???').to_s.sub(/^(?=\d)/, 'v')
    end
  ...
  def resolve_constraints(dependencies, source = [{:name => :you}], seen = {}, action = @action)
    dependencies = dependencies.map do |mod, range|
...

      {
        ...
        :path             => action == :install ? @options[:dir] : (@installed[mod].empty? ? @options[:dir] : @installed[mod].first.modulepath),
        :dependencies     => []
      }
    end.compact
