...
#   * Borrowed: Number of connections used
#   * Released: Number of connection released into the pool
#   * Closed: Number of closed connections (Might not appear if connections are closed after the report is sent)
...
#   * Service:
#   * Package:
#   * Graph Overhead: Catalog traverse time - Resources evaluation time
#   * Additionnal resource generation: Time to generate resource generated by other resources
#   * Catalog conversion: How logn it takes to convert the catalog into RAL
#   * Facter:
#   * Plugin Sync:
#   * Node retrieval: Time to gather node information like name or environment
#   * Reporting: (Only in last_run_summary.yaml and --summarize output) Time to send the report to the master
...
class Puppet::Transaction::Report
  ...
  # @api private
  def incr_metric(name, metric_name)
    if metric = @metrics[name]
      value = metric[metric_name]
      if value == 0
        metric.newvalue(metric_name, 1)
      elsif value !=0
        metric.replace(metric_name, value+1)
      else
        metric.newvalue(metric_name, 1)
      end
      metric
    else
      add_metric(name, {metric_name => 1})
    end
  end
