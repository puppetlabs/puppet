...
#     [data type](https://puppet.com/docs/puppet/latest/lang_data_type.html) that
#     must match the retrieved value. If not, the lookup (and catalog compilation)
#     fails. Defaults to `Data` (accepts any normal value).
...
#     merge behavior specified in the data sources. Defaults to no value; Puppet uses
#     merge behavior from the data sources if present, and otherwise performs a
#     first-found lookup.
...
#     no value; if Puppet can't find a normal value, the lookup (and compilation) fails.
...
# This is yet another way to set a default value for the lookup. If it finds no results,
# Puppet passes the requested key to the lambda and uses its result as the default value.
#
# #### Merge behaviors
#
...
# > request a specific merge behavior for a key. The `lookup` function uses that
...
#     values removed. The lookup fails if it finds any hash values.
# -   `'hash'` --- Combines the keys and values of any number of hashes to return a
#     merged hash. If the same key exists in multiple source hashes, Puppet uses the
#     value from the highest-priority data source. It doesn't recursively merge the
#     values.
# -   `'deep'` --- Combines the keys and values of any number of hashes to return a
#     merged hash. If the same key exists in multiple source hashes, Puppet recursively
#     merges hash or array values, removing duplicate values from arrays. For conflicting
#     scalar values, the highest-priority value wins.
# -   `{'strategy' => 'first'}`, `{'strategy' => 'unique'}`, or
#     `{'strategy' => 'hash'}` --- Same as the string versions of these merge behaviors.
...
#         to the prefix, it removes the entire element. Defaults to `undef`, which
...
#
# @example Perform a unique merge lookup of class names, then add all of those classes to the catalog (like `hiera_include`)
#
#     lookup('classes', Array[String], 'unique').include
#
# @example Perform a deep hash merge lookup of user data, but let higher priority sources remove values by prefixing them with `--`
#
# ``` puppet
# lookup( { 'name'  => 'users',
#           'merge' => {
