...
# ```
#
# This example produces the notice `true` if the looked-up hash has a value that is
# neither `false` nor `undef` for at least one of the keys. That is, it is
...
# ```
#
# This example produces the notice `true` if the looked-up hash has a value for one of
# the wanted keys that is neither `false` nor `undef`.
#
# When the lambda accepts two arguments, the first argument gets the index in an array
# or the key from a hash, and the second argument gets the value.
#
...
# ```
#
# The example produces the notice `true` because the index `2` is even and not a `String`.
#
# For general examples that demonstrate iteration, see the Puppet
