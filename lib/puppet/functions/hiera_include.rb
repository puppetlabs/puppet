...
Puppet::Functions.create_function(:hiera_include, Puppet::Functions::InternalFunction) do
  ...
  dispatch :hiera_include_block1 do
    scope_param
    param 'String',                       :key
    required_block_param 'Callable[1,1]', :default_block
  end

  dispatch :hiera_include_block2 do
    scope_param
    param 'String',                       :key
    param 'Any',                          :override
    required_block_param 'Callable[1,1]', :default_block
  ...
  def hiera_include_block1(scope, key, default_block)
    hiera_include_block2(scope, key, nil, default_block)
  end

  def hiera_include_block2(scope, key, override, default_block)
    undefined = (@@undefined_value ||= Object.new)
    result = lookup(scope, key, undefined, override)
    do_include(key, result.equal?(undefined) ? default_block.call(scope, key) : result)
  end
