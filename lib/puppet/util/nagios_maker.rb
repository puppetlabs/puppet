...
module Puppet::Util::NagiosMaker
  ...
  def self.create_nagios_type(name)
    ...
    type.newparam(nagtype.namevar, :namevar => true) do
      desc _("The name of this nagios_#{nagtype.name} resource.")
    end

    [ :owner, :group, :mode].each do |fileprop|
      type.newparam(fileprop) do
        desc _("The desired #{fileprop} of the config file for this nagios_#{nagtype.name} resource.
    ...
    nagtype.parameters.uniq.each do |param|
      ...
      type.newproperty(param) do
        desc _("Nagios configuration file parameter.")
      end
    end
    ...
    type.desc _("The Nagios type #{name.to_s}.  This resource type is autogenerated using the
