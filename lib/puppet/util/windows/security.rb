  ...
  def set_mode(mode, path, protected = true)
    ...
    set_acl(path, protected) do |acl|
      ...
      if isdir
        ...
        # the user has not specified SYSTEM for a group or owner
        if system_allow == FILE_ALL_ACCESS
          inherit = CONTAINER_INHERIT_ACE | OBJECT_INHERIT_ACE
          add_access_allowed_ace(acl, system_allow, Win32::Security::SID::LocalSystem, inherit)
        end
      end
    end
