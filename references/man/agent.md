...
## SYNOPSIS
Retrieves the client configuration from the Puppet server and applies it
to the local host.
...
a certificate request on the server, the certificate request the server
...
-   \--certname: Set the certname (unique ID) of the client. The server
