...
module Puppet
  module Acceptance
    module EnvironmentUtils
      ...
      def use_an_environment(environment, description, master_opts, confdir, config_opts = '')
        ssldir = on(master, puppet("master --configprint ssldir")).stdout.chomp
        options = master_opts.dup # shallow clone
        options[:__commandline_args__] = "--confdir=#{confdir} --ssldir=#{ssldir}"
        results = {}

        with_puppet_running_on(master, options, confdir) do
          agents.each do |agent|
            ...
            if agent == master
              ...
              on(agent, puppet(*(["config", "print", "basemodulepath", "modulepath", "manifest", "config_version", config_opts] + args)), :acceptable_exit_codes => (0..255)) do
