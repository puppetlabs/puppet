test_name "puppet module install (from an invalid tarball)"
...
step "Try to install a module from a nonexistent tarball"
on master, 'rm -f /tmp/pmtacceptance-stdlib-10.0.0.tar.gz'
on master, puppet("module install /tmp/pmtacceptance-stdlib-10.0.0.tar.gz"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    STDOUT> \e[mNotice: Reading metadata from '/tmp/pmtacceptance-stdlib-10.0.0.tar.gz' ...\e[0m
    STDERR> \e[1;31mError: Could not install from package /tmp/pmtacceptance-stdlib-10.0.0.tar.gz
    STDERR>   Package /tmp/pmtacceptance-stdlib-10.0.0.tar.gz does not exist\e[0m
  OUTPUT
end
on master, '[ ! -d /etc/puppet/modules/stdlib ]'

step "Try to install a module from an empty tarball"
on master, 'touch /tmp/pmtacceptance-stdlib-11.0.0.tar.gz'
on master, puppet("module install /tmp/pmtacceptance-stdlib-11.0.0.tar.gz"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    STDOUT> \e[mNotice: Reading metadata from '/tmp/pmtacceptance-stdlib-11.0.0.tar.gz' ...\e[0m
    STDERR> \e[1;31mError: Could not install from package /tmp/pmtacceptance-stdlib-11.0.0.tar.gz
    STDERR>   Error during extraction of module metadata: not in gzip format\e[0m
  OUTPUT
end
on master, '[ ! -d /etc/puppet/modules/stdlib ]'

step "Try to install a module from a tarball without metadata"
...
on master, puppet("module install /tmp/pmtacceptance-stdlib-12.0.0.tar.gz"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    STDOUT> \e[mNotice: Reading metadata from '/tmp/pmtacceptance-stdlib-12.0.0.tar.gz' ...\e[0m
    STDERR> \e[1;31mError: Could not install from package /tmp/pmtacceptance-stdlib-12.0.0.tar.gz
    STDERR>   The package is missing metadata file: metadata.json\e[0m
  OUTPUT
end
on master, '[ ! -d /etc/puppet/modules/stdlib ]'

step "Try to install a module from a tarball with invalid metadata"
...
on master, puppet("module install /tmp/pmtacceptance-stdlib-13.0.0.tar.gz"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    STDOUT> \e[mNotice: Reading metadata from '/tmp/pmtacceptance-stdlib-13.0.0.tar.gz' ...\e[0m
    STDERR> \e[1;31mError: Could not install from package /tmp/pmtacceptance-stdlib-13.0.0.tar.gz
