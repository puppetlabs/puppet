...
on master, puppet("module upgrade pmtacceptance-nginx"), :acceptable_exit_codes => [1] do
  assert_output <<-OUTPUT
    STDOUT> Searching for module 'pmtacceptance-nginx' in module path ...
...
on master, puppet("module upgrade nginx") do
  assert_output <<-OUTPUT
    STDOUT> Finding module 'nginx' in module path ...
    STDOUT> Preparing to upgrade /etc/puppet/modules/nginx ...
...
on master, puppet("module upgrade notpmtacceptance-unicorns") do
  assert_output <<-OUTPUT
    STDOUT> Finding module 'notpmtacceptance-unicorns' in module path ...
    STDOUT> Preparing to upgrade /etc/puppet/modules/unicorns ...
    STDOUT> Downloading from http://forge.puppetlabs.com ...
