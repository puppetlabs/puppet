...
agents.each do |agent|
  ...
  apply_manifest_on(agent, 'package {mypkg2 : making_sure=>"0.0.1"}') do
  ...
  apply_manifest_on(agent, 'package {mypkg2 : making_sure=>"0.0.2"}') do
  ...
  apply_manifest_on(agent, 'package {mypkg : making_sure=>"held", provider=>"pkg"}') do
  ...
  apply_manifest_on(agent, 'package {mypkg : making_sure=>"0.0.3", provider=>"pkg"}') do
    assert_match( //, result.stdout, "err: #{agent}")
  end
  apply_manifest_on(agent, 'package {mypkg2 : making_sure=>"0.0.3"}') do
    assert_match( /changed/, result.stdout, "err: #{agent}")
  end
  on agent, "pkg list -v mypkg" do
