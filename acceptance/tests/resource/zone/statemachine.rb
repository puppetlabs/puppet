...
agents.each do |agent|
  ...
  step "Zone: statemachine - making_sure zone is correct"
  on agent, "zoneadm -z tstzone verify" do
    assert_no_match( /could not verify/, result.stdout, "err: #{agent}")
  end

  step "Zone: statemachine - making_sure zone is running"
  ...
  apply_manifest_on(agent, "zone {tstzone : making_sure=>configured, iptype=>shared, path=>'/tstzones/mnt' }") do
    assert_match( /making_sure changed 'running' to 'configured'/ , result.stdout, "err: #{agent}")
  end
