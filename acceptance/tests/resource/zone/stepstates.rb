...
agents.each do |agent|
  ...
  apply_manifest_on(agent,"zone {tstzone : making_sure=>installed, iptype=>shared, path=>'/tstzones/mnt' }") do
    assert_match(/making_sure changed 'configured' to 'installed'/, result.stdout, "err: #{agent}")
  end

  step "Zone: steps - installed -> running"
  apply_manifest_on(agent,"zone {tstzone : making_sure=>running, iptype=>shared, path=>'/tstzones/mnt' }") do
    assert_match(/making_sure changed 'installed' to 'running'/, result.stdout, "err: #{agent}")
  end

  step "Zone: steps - running -> installed"
  apply_manifest_on(agent,"zone {tstzone : making_sure=>installed, iptype=>shared, path=>'/tstzones/mnt' }") do
    assert_match(/making_sure changed 'running' to 'installed'/, result.stdout, "err: #{agent}")
  end

  step "Zone: steps - installed -> configured"
  apply_manifest_on(agent,"zone {tstzone : making_sure=>configured, iptype=>shared, path=>'/tstzones/mnt' }") do
    assert_match(/making_sure changed 'installed' to 'configured'/, result.stdout, "err: #{agent}")
  end
end
