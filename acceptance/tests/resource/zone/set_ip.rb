...
agents.each do |agent|
  ...
  # --------------------------------------------------------------------
  apply_manifest_on(agent, 'zone {tstzone : making_sure=>configured, iptype=>exclusive, path=>"/tstzones/mnt" }') do
  ...
  step "Zone: ip - assign: making_sure that our ip assignment works."
  # --------------------------------------------------------------------
  apply_manifest_on(agent,'zone {tstzone : making_sure=>configured, iptype=>shared, path=>"/tstzones/mnt", ip=>"ip.if.1" }', :acceptable_exit_codes => [1] ) do
    assert_match(/ip must contain interface name and ip address separated by a \W*?:/, result.output, "err: #{agent}")
  end
  apply_manifest_on(agent, 'zone {tstzone : making_sure=>configured, iptype=>shared, path=>"/tstzones/mnt", ip=>"ip.if.1:1.1.1.1" }') do
    assert_match(/defined 'ip' as .'ip.if.1:1.1.1.1'./ , result.stdout, "err: #{agent}")
  end
  step "Zone: ip - assign: arrays should be created"
  # --------------------------------------------------------------------
  apply_manifest_on(agent, 'zone {tstzone : making_sure=>configured, iptype=>shared, path=>"/tstzones/mnt", ip=>["ip.if.1:1.1.1.1", "ip.if.2:1.1.1.2"] }') do
  ...
  apply_manifest_on(agent, 'zone {tstzone : making_sure=>configured, iptype=>shared, path=>"/tstzones/mnt", ip=>["ip.if.1:1.1.1.1", "ip.if.2:1.1.1.3"] }') do
