test_name "Calling Hiera function from inside templates" do
  tag 'audit:medium',
      'audit:integration'

  agents.each do |agent|
    @module_name = "hieratest"
    @coderoot = agent.tmpdir(@module_name.to_s)
    ...
    apply_manifest_on(agent, env_manifest, catch_failures: true)
    ...
    on(
      agent,
      puppet("apply",
             "#{@coderoot}/environments/production/manifests/site.pp",
             confdir: "#{@coderoot}/puppet"), acceptable_exit_codes: [0, 2]
    )
