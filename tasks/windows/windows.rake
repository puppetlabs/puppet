...
namespace :windows do
  ...
  FileList[File.join(File.expand_path(File.dirname(__FILE__)), 'stagedir', '*')].each do |fn|
    my_topdir = File.basename(fn)
    WXS_FRAGMENTS_HASH[my_topdir] = { :src => "stagedir/#{my_topdir}" }
    file "stagedir/#{my_topdir}" => ["stagedir"] do |t|
      puts "Copying #{File.dirname(__FILE__)}/#{t.name} to the staging area ..."
      src = File.join(File.expand_path(File.dirname(__FILE__)), t.name)
      dst = t.name
